...
const initial_form_data = {
  megaEventExperienceDetails: "",
  megaEventExperienceDetailsValid: false,
};

export default function MegaEventExperienceDetails() {
  ...
  const handleInputChange = (event) => {
    ...
    setFormData({
      ...formData,
      megaEventExperienceDetails: value,
      megaEventExperienceDetailsValid: value.length >= 1,
    });
  };
      ...
      <div className="relative w-8/12 flex flex-col bg-white">
        ...
        <div className="w-full h-screen max-w-[512px] mx-auto flex flex-col justify-center gap-y-8">
          ...
          <div className="space-y-6">
            {/* Form */}
            <form action="">
              {/* Mega Event Experience Details Row */}
              <label
                className="text-gray-700 text-xs-medium 2xl:text-sm-medium sr-only"
                htmlFor="megaEventExperienceDetails"
              >
                Mega Event Experience Details<span className="text-primary-600">*</span>
              </label>
              <div className="w-full">
                <textarea
                  id="megaEventExperienceDetails"
                  name="megaEventExperienceDetails"
                  rows="4"
                  value={formData.megaEventExperienceDetails}
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
              ...
              <button
                ...
                disabled={!(formData.megaEventExperienceDetails)}
                onClick={() => router.push("/volunteers-form/team-lead-experience-choice")}
              >
                Continue
              </button>
            </div>
