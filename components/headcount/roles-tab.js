...
import { getRole, getFunction } from '@/pages/api/get';
...
import { addRoleFile } from '@/pages/api/post';
...
const column_options = [
  { name: 'Role name', isChecked: false },
  { name: 'Role description', isChecked: false },
  { name: 'Role code', isChecked: false },
  { name: 'Workforce type', isChecked: false },
  { name: 'Function name', isChecked: false },
];
...
function RolesTab() {
  ...
  if (router_query.item !== undefined) {
    function_guid = JSON.parse(router_query.item).function_guid;
  }
  ...
  console.log(functions, 'fucntions..');
    ...
    if (roles?.length > 0) {
      const obj = [
        ...
        },
        {
          ...
          filterOptions: roles
            ?.filter((item) => item.role_description)
            ?.map((item) => ({
              name: item.role_description,
              isChecked: false,
            })),
        },
        {
        ...
        },
        {
          ...
          filterOptions: roles
            ?.filter((item) => item.workforce_type)
            ?.map((item) => ({
  ...
  useEffect(() => {
    getMyRoles();
    getMyFunctions();
    setQueryParamFunction(router?.query);
  }, []);
  ...
  const [query, setQueryParamFunction] = useState(null);

  /**
   * Filter export data on row traversal
   */
  const HandleRowClickExportData = () => {
    const function_guid = JSON.parse(query.item).function_guid;
    const functionRoles = roles.filter(
      (item) => item.FunctionGUID === function_guid
    );
    handleExport(
      functionRoles,
      ['role_code', 'role_name', 'role_description', 'workforce_type'],
      'roles.xlsx'
    );
  };
  ...
  const handleAttatchmentChange = async (event) => {
    ...
    let res = await addRoleFile(formData);
    let { valid } = res;
    if (!valid) {
  ...
  const handleEditModal = (data) => {
    setShowRoleModal(data?.showModal);
  ...
  const handleQuery = (item) => {
    if (!function_guid) return roles;
    else if (item.function_guid === function_guid) {
      return true;
    } else {
      return false;
    }
          ...
          <form className='w-full space-y-6'>
            ...
            <div className='w-full bg-gray-50 border-x border-t border-gray-200 rounded-2xl shadow-sm'>
              ...
              <RolesTable
                ...
                getMyRoles={getMyRoles}
                roles={roles?.filter(handleQuery)}
              ...
              <div className='w-[352px] mx-auto flex flex-col gap-y-6 items-center justify-center'>
                <div className='w-full flex flex-col justify-center items-center gap-y-4'>
                  ...
                  <p className='text-xs-regular 2xl:text-sm-regular text-gray-600 text-center max-w-[38ch]'>
                    Looks like you haven't added any roles yet. Click the "Add
                    Role" button to get started!
                  </p>
                </div>
                {/* Buttons */}
